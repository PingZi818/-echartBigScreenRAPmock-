...
function selectArea() {
    $.get('resource/data/data.json').done(function (data) {
        ...
        $('.provinceContainer').mouseleave(function () {
...
function activeTeacher(areaCode) {
    ...
    $.get(BACK_ROOT + '/bigscreen/interact/activeteacherdata.do?areaCode='+areaCode).done(function (data) {
        ...
        activeTeacherInterval = hoverTurn(option1,myChart1,-1);
        // clearIntervals('.acting-teacher .com-screen-content',activeTeacherInterval,option1,myChart1);
    ...
    $.get(BACK_ROOT + '/bigscreen/interact/subareadata.do?areaCode='+areaCode).done(function (data) {
        ...
        var  adminTeachOption= {
            tooltip: {
                ...
                formatter: function(params){
                    var name = params.data.name ? params.data.name : '';
                    var res = name+'<br/>';
        ...
        adminTeachCountInterval=hoverTurn(adminTeachOption,myChart2,-1);
        // clearIntervals('.admin-teach-data-count  .com-screen-content',adminTeachCountInterval,adminTeachOption,myChart2);
        var Tpl='',Tpls='';
        if(data.length>0){
            for(var i=0;i<6;i++){
...
function teachCountPic(year,areaCode){
    ...
    $.get(BACK_ROOT + '/bigscreen/interact/trendstaticdata.do?year='+year+'&areaCode='+areaCode).done(function (data) {
        ...
        teachCountPicInterval = hoverTurn(teachCountPicOption,myChart3,-1);
        // clearIntervals('.teach-count-pic .com-screen-content',teachCountPicInterval,teachCountPicOption,myChart3);
    });
...
function jiaoan(result){
    ...
    if(result !=null){
        ...
        jiaoanInterval = hoverTurn(teachResearchOption1,myChart4,-1);
        // clearIntervals('#teach_research1',jiaoanInterval,teachResearchOption1,myChart4);
    }else{
        myChart4.clear();
